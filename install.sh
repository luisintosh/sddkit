...
suggest_next_steps() {
  ...
  log ""
  log "Optional: sddkit-ship — runs every feature of that roadmap from its GitHub"
  log "  issues, one sddkit run at a time in this checkout, auto-merging green PRs."
  log "  Needs a clean working tree and the OpenCode CLI as its runner (even from"
  log "  Cursor's /sddkit-ship skill)."
  log ""
