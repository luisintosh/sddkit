...
## Inputs
- The slice brief from `@sdd` (the slice's section from `plan.md`, `@S<n>` scenario text, test command) — prefer it over re-reading `plan.md` in full; read from disk only if the brief is missing or ambiguous, or the failure history warrants a closer look
